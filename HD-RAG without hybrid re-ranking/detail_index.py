...
def load_or_create_detail_index(pages, persist_directory="./faiss_indexes", collection_name="detail_index"):
    ...
    if os.path.exists(index_path) and os.path.exists(metadata_path) and os.path.exists(ids_path) and os.path.exists(documents_path):
        print(f"Loading existing FAISS detail index: {collection_name}")
    ...
    print(f"Creating new FAISS detail collection: {collection_name}")
    ...
    # 创建文本块
    all_chunks = []
    for page in tqdm(pages, desc="Creating text chunks"):
    ...
    print(f"Added {len(all_chunks)} chunks to FAISS detail index")
...
def detail_search(collection, query, k=5, filter_metadata=None):
    embedding_model = get_embedding_model()
    ...
    # 使用FAISS进行相似性搜索
    distances, indices = collection['index'].search(query_vector, k * 2)  # 检索更多结果以支持过滤
